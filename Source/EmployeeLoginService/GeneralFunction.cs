...
namespace EmployeeLoginService
{
    public class GeneralFunction
    {
            ...
            using (var context = new ELDBDataContext())
            {
                ...
                if (ValidUserId != 0)
                {
                    ...
                    if (CheckUser.First().RoleId == 1)
                    {
                        ...
                        LD.EmailAddress = (from u in context.Users where u.UserId == ValidUserId select u.EmailID).FirstOrDefault();
                        LD.UDId = -1;
                    }
                    else
                    {
        ...
        internal List<HierarchyUserDetail> GetChildren(ELDBDataContext context, int parentId)
        {
        ...
        internal bool ShowAllCompanyDetail(ELDBDataContext context, int UserId)
        {
